...
Search Stories is an app that can search stories and return a story that matches in the indexed data using Jina
...
```bash
docker build -t jinaai/hub.app.search_stories:0.0.1 .
```

To mount local directory and run:
```bash
docker run -v "$(pwd)/j:/workspace" jinaai/hub.app.search_stories:0.0.1
``` 

To query
```bash
docker run -p 65482:65482 -e "JINA_PORT=65481" jinaai/hub.app.search_stories:0.0.1 search
```
...
Copyright (c) 2020 Sibish Basheer. All rights reserved.
